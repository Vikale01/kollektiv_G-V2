...
/**
  * @brief BMI270 I2C write-wrapper
  * @param memAddress Internal memory adress
...
/**
  ...
  * @retval None
  */
static int BMI270_I2C_read(uint8_t memAddress, uint8_t *pData)
{
    if (HAL_I2C_Mem_Read(&BMI270_I2C, 
                         BMI270_ADDRESS, 
                         memAddress, 
                         1, 
                         pData, 
...
/**
  * @brief BMI270 write configuration function
  * @param None
  * @retval None
  */
static int BMI270_WriteConfig(void)
{
    uint8_t chip_id;
    uint8_t status_value = 0x00;
    uint8_t dataBuffer = 0x00;

	// Check the chip ID before reading
    if ((BMI270_I2C_read(REG_CHIP_ID_BMI270, &chip_id)) != HAL_OK)
    {
        return BMI270_ERR_I2C;
    }
 
	if (chip_id != CHIP_ID) { return BMI270_ERR_CHIP_ID; }

    dataBuffer = SOFT_RESET_CMD;
    if(HAL_I2C_Mem_Write(&BMI270_I2C, BMI270_ADDRESS, REG_CMD, 1, &dataBuffer, 1, 1000) != HAL_OK)
    {
        return BMI270_ERR_I2C;
    }
    HAL_Delay(10); 
    ...
    dataBuffer = 0x00;
    //Power config   
    if(HAL_I2C_Mem_Write(&BMI270_I2C, BMI270_ADDRESS, REG_PWR_CONF, 1, &dataBuffer, 1, 1000) != HAL_OK)
    {
        return BMI270_ERR_I2C; //I2C FEL
    }
    HAL_Delay(1);

    //Init ctrl
    dataBuffer = 0x00;
    if(HAL_I2C_Mem_Write(&BMI270_I2C, BMI270_ADDRESS, REG_INIT_CTRL, 1, &dataBuffer, 1, 1000) != HAL_OK)
    {
        return BMI270_ERR_I2C; //I2C FEL
    }
    HAL_Delay(1);

    if(HAL_I2C_Mem_Write(&BMI270_I2C, BMI270_ADDRESS, REG_INIT_DATA, I2C_MEMADD_SIZE_8BIT, (uint8_t *)bmi270_config_file, 8192, 10000) != HAL_OK)
    {
        return BMI270_ERR_I2C; //I2C FEL
    }
    HAL_Delay(1);
    ...
    while (timeout > 0)
    {
        if (HAL_I2C_Mem_Read(&BMI270_I2C, BMI270_ADDRESS, REG_INTERNAL_STATUS, 1, &status_value, 1, 1000) != HAL_OK)
        {
            return 1; 
        }
...
int BMI270_Configure()
{
    ...
    if(BMI270_WriteConfig() != 0)
    {
        return 1;
    }

    reg_value = 0x0E;
    if(HAL_I2C_Mem_Write(&BMI270_I2C, BMI270_ADDRESS, REG_PWR_CTRL, 1, &reg_value, 1, 1000) != HAL_OK)
    {
        return 1;
    }
    HAL_Delay(10);
    ...
    reg_value = 0xA8;
    if(HAL_I2C_Mem_Write(&BMI270_I2C, BMI270_ADDRESS, REG_ACC_CONF, 1, &reg_value, 1, 1000) != HAL_OK)
    {
        return 1;
    }
    HAL_Delay(1);
    ...
    reg_value = 0x01;
    if(HAL_I2C_Mem_Write(&BMI270_I2C, BMI270_ADDRESS, REG_ACC_RANGE, 1, &reg_value, 1, 1000) != HAL_OK)
    {
        return 1;
    }
    HAL_Delay(1);
    ...
    reg_value = 0xA9;
    if(HAL_I2C_Mem_Write(&BMI270_I2C, BMI270_ADDRESS, REG_GYR_CONF, 1, &reg_value, 1, 1000) != HAL_OK)
    {
        return 1;
    }
    HAL_Delay(1);
    ...
    reg_value = 0x02;
    if(HAL_I2C_Mem_Write(&BMI270_I2C, BMI270_ADDRESS, REG_PWR_CONF, 1, &reg_value, 1, 1000) != HAL_OK)
    {
        return 1;
    }
    HAL_Delay(1);
...
/**
  * @brief BMI270 read measurement function
  * @param None
  * @retval Accelerometer and Gyro values
  */
void BMI270_ReadMeasurement(float *ax, float *ay, float *az, float *gx, float *gy, float *gz)
{
    uint8_t sensor_raw[12];

    if(HAL_I2C_Mem_Read(&BMI270_I2C, BMI270_ADDRESS, REG_ACC_GYRO_DATA, 1, sensor_raw, 12, HAL_MAX_DELAY) != HAL_OK)
    {
        *ax = *ay = *az = *gx = *gy = *gz = 0.0f;
        return;
    }

    raw_gx = ((sensor_raw[1] << 8) | sensor_raw[0]);
    raw_gy = ((sensor_raw[3] << 8) | sensor_raw[2]);
    raw_gz = ((sensor_raw[5] << 8) | sensor_raw[4]);

    raw_ax = ((sensor_raw[7] << 8) | sensor_raw[6]);
    raw_ay = ((sensor_raw[9] << 8) | sensor_raw[8]);
    raw_az = ((sensor_raw[11] << 8) | sensor_raw[10]);
