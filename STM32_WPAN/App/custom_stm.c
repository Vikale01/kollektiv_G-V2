...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomDsHdle;                    /**< dataSend handle */
  uint16_t  CustomDpHdle;                  /**< dataPackage handle */
...
#define COPY_DATASEND_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_DATAPACKAGE_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomDpHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_DP_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_DP_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for dataSend +
   *                                2 for dataPackage +
   *                                1 for dataPackage configuration descriptor +
   *                              = 4
   *
  ...
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             ...
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomDsHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: DS, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: DS , handle = 0x%04x \n\r", CustomContext.CustomDsHdle);
  }

  /**
   *  dataPackage
   */
  COPY_DATAPACKAGE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomDsHdle,
                          UUID_TYPE_128, &uuid,
                          SizeDp,
                          CHAR_PROP_NOTIFY,
                          ...
                          &(CustomContext.CustomDpHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : DP, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : DP , handle = 0x%04x \n\r", CustomContext.CustomDpHdle);
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_DP:
      ret = aci_gatt_update_char_value(CustomContext.CustomDsHdle,
                                       CustomContext.CustomDpHdle,
                                       0, /* charValOffset */
                                       SizeDp, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value DP command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value DP command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_DP:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomDsHdle, CustomContext.CustomDpHdle, SizeDp, pPayload);
